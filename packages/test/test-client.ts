...
function handleResponse(message: BaseMessage): void {
	...
	if (
		...
		pendingRequest.reject(
			new Error(`Server error: ${JSON.stringify(message)}`),
		);
	} else {
...
async function runTests(): Promise<void> {
	try {
		...
		const modelsResponse = await sendRequest<ModelsResponseMessage>(
			MessageType.MODELS_REQUEST,
		);
		console.log("Models response:", JSON.stringify(modelsResponse, null, 2));

		// Test 2: Simple chat completion
		console.log("\nTEST 2: Simple chat completion");
		const chatResponse = await sendRequest<ChatResponseMessage>(
		...
		const chunkMessage = createMessage<StreamChunkMessage>(
		...
		setTimeout(() => {
			...
			const endMessage = createMessage<StreamEndMessage>(
...
function handleStreamChunk(message: StreamChunkMessage): void {
	...
	try {
		...
		log(
			`Data preview: ${data.substring(0, 100)}${data.length > 100 ? "..." : ""}`,
		);
	}
	...
	if (pendingRequest) {
		log(
			`Received chunk for pending request: ${requestId} (type: ${pendingRequest.type})`,
		);
	} else {
